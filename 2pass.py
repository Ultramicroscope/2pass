...
try:
    ...
    subprocess.run(['ffmpeg', *(c := ['-y', '-i', f_in, '-c:v', 'libx264', '-b:v', str(8 * (1006632 * parser.parse_args().size - f_in.stat().st_size + Path(temp).stat().st_size) / float(subprocess.check_output(['ffprobe', '-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', f_in], text=True))), '-passlogfile', f'{f_in}_ffmpeg2pass']), '-pass', '1', '-an', '-f', 'mp4', os.devnull], check=True)
